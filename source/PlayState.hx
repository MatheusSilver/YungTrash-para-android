...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			if(ClientPrefs.cenoptim){
				...
				var bg:BGSprite = new BGSprite('miner/minerocean', 0, 0, 0.7, 0.7);
				bg.setGraphicSize(Std.int(bg.width * 1.0));
				bg.updateHitbox();
				add(bg);	

				var bg:BGSprite = new BGSprite('miner/minermountain2', 0, 0, 0.9, 0.9);
				bg.setGraphicSize(Std.int(bg.width * 1.0));
				bg.updateHitbox();
				add(bg);

				var bg:BGSprite = new BGSprite('miner/minerforest', 0, 0, 0.8, 0.8);
				bg.setGraphicSize(Std.int(bg.width * 1.0));
				bg.updateHitbox();
				add(bg);
			
				var bg:BGSprite = new BGSprite('miner/minermountain', 0, 50, 0.9, 0.9);
				bg.setGraphicSize(Std.int(bg.width * 1.0));
				bg.updateHitbox();
				add(bg);	

				var bg:BGSprite = new BGSprite('miner/minerstage', 0, 15, 1.0, 1.0);
				bg.setGraphicSize(Std.int(bg.width * 1.0));
				bg.updateHitbox();
				add(bg);
			...
			if(ClientPrefs.cenoptim){
				...
				var bg:BGSprite = new BGSprite('chicken/sky', -150, 0, 0.8, 0.8);
				bg.setGraphicSize(Std.int(bg.width * 1.6));
				bg.updateHitbox();
				add(bg);

				var bg:BGSprite = new BGSprite('chicken/mountain', 300, 100, 0.7, 0.7);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.updateHitbox();
				add(bg);

				var bg:BGSprite = new BGSprite('chicken/nuvens', 350, 0, 0.8, 0.8);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.updateHitbox();
				add(bg);

				var bg:BGSprite = new BGSprite('chicken/celeiro', 900, 20, 0.8, 0.8);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.updateHitbox();
				add(bg);

				var bg:BGSprite = new BGSprite('chicken/cerca', 75, 475, 1.0, 1.0);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.updateHitbox();
				add(bg);

				var bg:BGSprite = new BGSprite('chicken/grass', -675, 600, 1.0, 1.0);
				bg.setGraphicSize(Std.int(bg.width * 1.6));
				bg.updateHitbox();
				add(bg);
		...
		if (isStoryMode && !seenCutscene && !ClientPrefs.dadia)
		{
			switch (daSong)
			{
				case "monster":
	...
	#if mobileC // stupid way to fix keyshit for mobile bruh
	private function keyShit():Void
	{
		...
		if (!boyfriend.stunned && generatedMusic)
		{
			if((left || down || up || right) && !endingSong) {
				...
				#if ACHIEVEMENTS_ALLOWED
				var achieve:Int = checkForAchievement([11]);
				if (achieve > -1) { //Malz marcos, isso foi nescessario
					startAchievement(achieve);
				}
				#end
			...
			if((leftP || downP || upP || rightP) && !endingSong) {
				...
				//var canMiss:Bool = !ClientPrefs.ghostTapping; // temp 
